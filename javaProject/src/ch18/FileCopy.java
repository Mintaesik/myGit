...
public class FileCopy {
	public static void main(String[] args) {
		//복사본 <==  (OutputStream객체)  [프로그램]   <==  (InputStream객체 사용) 원본
		String str = "";
		...
		try {
			...
			try {
				if(reader != null) reader.close(); //리소스를 개별적으로 닫아주는게 좋다. reader는 리소스
			} catch (Exception e2) {
				e2.printStackTrace();
			}
			try {
				if(writer != null) writer.close(); //리소스 닫아주기
			} catch (Exception e2) {
				e2.printStackTrace();
			}
		}
