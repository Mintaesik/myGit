...
//MouseListener : 마우스인벤트 처리를 위한 인터페이스
//MouseMotionListener : 마우스 이동 이벤트 처리를 위한 인터페이스
//KeyListener : 키보드이벤트 처리를 위한 인터피에스
...
public class KeyMouseExam extends JApplet implements MouseListener, MouseMotionListener, KeyListener{
	...
	public void init() {
		...
		this.addMouseListener(this); //현재 클래스에 마우스 이벤트 기능을 추가
	...
	public void keyPressed(KeyEvent e) {
		...
		switch (e.getKeyCode()) { //키코드값에 따라 분리
		...
		case KeyEvent.VK_DOWN:
			y=Math.min(300-height, y+5); break; //프레임사이즈 (300,300)
	...
	public void mouseClicked(MouseEvent e) {
		flag = true; //클릭할때 그림을 true로 설정되게 함.
		//마우스 클릭한 좌표값 저장
		x = e.getX();
		y = e.getY();
	...
	public void mousePressed(MouseEvent e) {
		//컴퍼넌트상에서 마우스 버튼을 누르는 시점 때에 허출
		
	}

	@Override
	public void mouseReleased(MouseEvent e) {
		//컴퍼넌트상에서 마우스를 떼어 놓아질때 호출
